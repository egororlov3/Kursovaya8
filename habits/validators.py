...
def validate_duration(time_to_complete):
    if time_to_complete is None:
        raise (ValidationError
               ("Время на выполнение должно быть задано и не может быть пустым."))

    if time_to_complete <= 0:
        raise (ValidationError
               ("Время на выполнение должно быть положительным числом."))

    if time_to_complete > 120:
        raise (ValidationError
               (_("Продолжительность не должна превышать 120 секунд.")))


def validate_related_habit_is_pleasurable(linked_habit):
    if linked_habit and not linked_habit.is_pleasurable:
        raise (ValidationError
               (_("Связанной может быть только приятная привычка.")))


def validate_pleasurable_habit(is_pleasant, reward, linked_habit):
    if is_pleasant and (reward or linked_habit):
        raise (ValidationError
               (_("Приятная привычка не может иметь вознаграждений или связанных привычек.")))


def validate_frequency(period):
    if period > 7:
        raise (ValidationError
               (_("Частота выполнения привычки должна быть не реже одного раза в 7 дней.")))
